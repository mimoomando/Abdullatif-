...
def run_once(bridge, cfg: RunConfig, recorder: Recorder,
    ...
    for poi_tf, confirm_tf in cfg.pairs.items():
        try:
            ...
            # ⛔⛔⛔ **وشرطُ الجلب كان `higher_poi_needed(poi_tf)`، فقتل
            #    بوّابةَ الاتّجاه على H1 قتلًا صامتًا** (كُشف 09-25):
            #
            #    `HIGHER_FRAME` يقول `H1 ⇒ H4`، و`higher_poi_needed("H1")`
            #    يقول `False` — فالسطرُ في الجدول **ميّت**، ولا يُجلب H4
            #    قطّ، و`evaluate` يتلقّى `higher_series=None`.
            #
            # وهما **قاعدتان مختلفتان** خُلطتا:
            #
            #   ① سندُ نقطة الاهتمام — «الأوردر بلوك على أربع ساعات **ما
            #     بحاجة**… أما اللي على الربع ساعة **بحاجة** لنقاط اهتمام
            #     من إطار أكبر». فهذه تخصّ الفريمات الصغيرة وحدها.
            #
            #   ② موافقةُ الاتّجاه — «إذا انت فايت **عكس الترند** رح
            #     يضربك». وهذه **لا تستثني فريمًا**.
            #
            # فأُغلقت ② بشرطِ ①. و`chain.py:543` يحرس ① بنفسه بـ
            # `higher_poi_needed`، فجلبُ الإطار الأعلى لا يفرض سندًا على
            # H1 — يفتح بوّابةَ الاتّجاه وحدها.
            #
            # 🔴 وكلفتُه مقيسة في أسبوع 21–25/9: صفقتا H1 كلتاهما
            #    **شراءٌ**، وH4 هابطٌ بوضوح (4377 ⇐ 4273)، وكلتاهما
            #    ضُربت — **−27.22$**، وهي الخسارةُ كلُّها وزيادة.
            higher = None
