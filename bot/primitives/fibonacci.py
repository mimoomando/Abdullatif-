...
class Impulse:
    ...
    def is_expensive(self, price: float, side: Direction) -> bool:
        ...
        ⚠️ يقيس **مستوى سعر** مجرَّدًا. للحكم على شمعة استعمل
        `closed_beyond_midpoint()` — فالبثّ المباشر نصّ على أن الفاصل
        الإغلاق لا اللمس.
        """
        if side == "bullish":
    ...
    def closed_beyond_midpoint(self, candle, side: Direction) -> bool:
        """
        هل تجاوزت الشمعة بوابة الـ50% **بالإغلاق**؟

        منصوص في البثّ المباشر:

            «التصحيح وصل 50% ماكسيموم تتاخذ… ما قلت لك الماكسيموم
             ماكسيموم ماكسيموم 50%؟ **بس ما يغلق** — هي **مش غالق**
             فوق الـ50%»

        ⇒ **الذيل فوق المنتصف مقبول · والإغلاق فوقه مرفوض.**

        وهذا يوافق قاعدته العامة «الكسر بالجسم لا بالذيل» (الدرس 10):
        الذيل يستكشف، والإغلاق يُقرّر.
        """
        return self.is_expensive(candle.close, side)
