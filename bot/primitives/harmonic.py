...
# ⭐⭐⭐ الجدول — **كاملًا من بطاقته المنشورة** (2026-09-20).
#
# قال في درس الفراشة: «ما تعذّب حالك إنك تاخذ سكرين شوت… كل ما عملنا
# لك، عليك تروح على **صندوق الوصف بالفيديو**». وفي وصف درس البرق
# السريع ثلاثة روابط، أحدها «نسب البرق السريع» — وهي هذه البطاقة:
#
#     تصحيح C          امتداد D      SL
#     ────────────────────────────────────
#     0.382 – 0.48       2.240      2.618
#     0.49  – 0.59       2.000      2.240
#     0.60  – 0.68       1.618      2.000
#     0.69  – 0.76       1.410      1.618
#     0.77  – 0.86       1.270      1.410
#     0.87  – 0.90       1.130      1.270
#
# ⛔ **وأسقطت البطاقةُ صفًّا كان خطأً عندنا.** كان السطر الخامس
# `(0.85, 1.13)` — مبنيًّا على مثالٍ واحد سمعناه (0.85 ⇒ 1.13)،
# وكان يجمع صفّين في واحد ويعطي **الامتداد الخطأ**:
#
#     تصحيح 0.80   ⇐ كنّا نعطي 1.130   والصواب **1.270**
#     تصحيح 0.88   ⇐ كنّا نرفضه        والصواب **1.130**
#
# ⇒ فكلّ نموذجٍ تصحيحُه بين 0.77 و0.86 كان دخولُه محسوبًا خطأً.
# 🔴 **H1 مغلق.**
#
# ⚠️ والجدول بخانتين عشريّتين، فبين الصفوف فجواتٌ (0.48 ← 0.49).
# وتصحيحٌ يقع فيها (0.485) يُرفَع إلى الصفّ الأعلى — وهو ما يفعله
# `bisect_right` أصلًا، ولا رقم من عنده.
#
...
# الرابحة. **H2 مغلق.**
# ⛔ **و1.270 كانت غائبةً من السلّم كلِّه** حتى وصلت البطاقة. فوقفُ
# الدخول من 1.130 كان يُحسب 1.410 (الدرجة التالية في سلّمٍ ناقص)،
# والبطاقة تقول **1.270** — أي وقفٌ **أضيق** مما كنّا نضعه.
LADDER: Tuple[float, ...] = (1.130, 1.270, 1.410, 1.618, 2.000, 2.240, 2.618)

# ⭐ ولم تعد هذه مشتقّةً من السلّم — **عمود SL في البطاقة نصًّا**.
...
# ⭐⭐ **منطقة الانعكاس المحتملة — من بطاقته المنشورة** (2026-09-20).
#
# ثالث رابطٍ في وصف درس البرق السريع عنوانه «منطقة PRZ»، وهو **لقطةٌ
# لإعدادات أداة الفيبوناتشي عنده**. والمؤشَّر فيها أربعة مستويات لا
# غير:
#
#         ✅ 0      ✅ 1      ✅ 1.27      ✅ 1.618
#
# وكلّ ما عداها مُطفأ (0.382 · 0.5 · 0.62 · 0.786 · 0.79 · 1.272 ·
# 1.414 · 2 · 2.272 …) — فالاختيار مقصود.
#
# 🔶 **والقراءة استنتاجٌ لا نصّ.** البطاقة تعطي **الأرقام**، وقولُه
# يعطي **المرساتين**: «الـPRZ بيتاخذ **من B إلى A**». فـ0 عند A و1
# عند B، والنطاق بين 1.27 و1.618 خلفها.
#
# ⭐ **وفحصٌ كان يمكن أن يسقط ولم يسقط**: D المحسوبة من الجدول تقع
# داخل هذا النطاق في **ستّة صفوفٍ من ثمانية**. والخارجان هما
# العميقان (تصحيح 0.81 و0.89) — وذلك يوافق قولَه إنّ السعر «**أحيانًا
# كثير ما بيوصل لنقطة الدخول، بيرتد من الـPRZ زون**»، لا يناقضه.
#
# 🔴 **H3 ضُيِّق لا أُغلق**: الأرقام معلومة، والقراءة مرجَّحة.
PRZ_LEVELS: Tuple[float, float] = (1.27, 1.618)


def prz_from(a_price: float, b_price: float) -> Tuple[float, float]:
    """
    نطاق الانعكاس المحتمل — فيبوناتشي **من B إلى A**، بين 1.27 و1.618.

    ⛔ **ولا يُدخَل منه.** منصوص:

        «ما تعتمد كثير على البي ار زد — ليه؟ لأنه راح يعطيك مناطق
         انعكاس ولكن راح **يكبّر لك الستوب**… فكارثي بالنسبة لمتداول
         على الذهب»

    فيُرجَع للعرض والسياق، والدخول يبقى عند D.
    """
    leg = b_price - a_price
    edges = (a_price + leg * PRZ_LEVELS[0], a_price + leg * PRZ_LEVELS[1])
    return (min(edges), max(edges))


# «بتحط عندك نسب 0.382 · 0.5 · 0.618» + «بدك هدف سريع حط الـ0.236»
#
# ✅ **وأكّدتها بطاقتُه المنشورة «نسب الأهداف»** (2026-09-20):
# المؤشَّر فيها ✅0 · ✅0.382 · ✅0.5 · ✅0.618 · ✅1 — الطرفان
# مرساتان، والثلاثة الوسطى **برتقاليّة** (لونُ ما يتصرّف عليه،
# كنسب OTE على شارته الحيّ). و**0.236 غير مؤشَّرة**، فهي إضافةٌ
# لا أصل — ولذلك `include_fast=False` افتراضًا.
...
class FastLightning:
    ...
    def prz(self) -> Tuple[float, float]:
        """نطاق الانعكاس المحتمل — من B إلى A، بين 1.27 و1.618."""
